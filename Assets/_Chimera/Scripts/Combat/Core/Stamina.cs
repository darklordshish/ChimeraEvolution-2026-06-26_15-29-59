...
public class Stamina : MonoBehaviour
{
    ...
    /// <summary>НАГРУЗКА (погоня, бег) — в отличие от `TrySpend` НЕ отодвигает реген: они идут параллельно и
    /// сходятся в равновесие. Это принципиально: непрерывный расход через `TrySpend` опустошает бак ВСЕГДА
    /// (реген при нём не работает ни секунды), и полоска просто переезжает жить на ноль — стамина
    /// превращается в дебафф вместо риск-менеджмента.
    ///
    /// Разделение: РЫВОК УСИЛИЯ (таран, рывок, прыжок) — `TrySpend`, он и должен сбивать дыхание;
