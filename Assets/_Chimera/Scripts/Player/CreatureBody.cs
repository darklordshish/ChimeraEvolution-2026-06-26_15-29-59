...
public partial class CreatureBody : MonoBehaviour
{
    ...
    void Recompute()
    {
        ...
        if (chassis != null && chassis.sockets != null && chassis.sockets.Length > 0)
        {
            ...
            // КЛЕТКА СЮДА ПОКА НЕ ПРИХОДИТ. Смеситель `GetBlendedCages` остался и покрыт тестами, но
            // потребителя у него нет: сборка меша по таблице (`MorphBuilder.BuildCage`) откачена 08.09 —
            // она строила слоты независимыми трубами и имитировала узел погружением, то есть ровно тем
            // способом, который `SPEC-telo-kak-graf.md` §1 называет ПРИЧИНОЙ разошедшихся швов, а не лечением.
            //     Порядок восстановления обратный тому, как это писали: сперва узлы-многогранники в
            // `cagemesh.py`, затем ландмарки из общего графа в обмерщик, затем экспортёр
            // `species/*_cage.py → SpeciesSO.cages` (его не существует, оттого во всех пяти ассетах
            // `cages: []`), и только потом сборка здесь — по уже сшитой геометрии она куда проще
