...
public static class BoneMesher
{
    ...
    static (string slot, Mesh mesh)[] Polygonize(List<Seg> segs, Matrix4x4[] bind, float cell, float blend, float fur)
    {
        ...
        //     УЗЛЫ СЕТКИ — НА ±ПОЛКЛЕТКИ, А НЕ НА ОСИ (23.09). Узел на оси делал середину каждого тела полосой граней
        // шириной в клетку, смотрящей вперёд: на лбу волка и лице человека читалась светлая планка в 4 см. Со сдвигом
        // ячейка оси стоит поперёк неё, её вершина ложится на X = 0, и две грани сходятся ребром — как спинка носа
        float halfX = (Mathf.Ceil(Mathf.Max(-lo.x, hi.x) / cell - 0.5f) + 0.5f) * cell;
        lo.x = -halfX; hi.x = halfX;
        // РЕШЁТКА ПО Y И Z ПОСТОЯННАЯ — узлы на кратных клетки в системе тела, а не от габарита (24.09, находка модельной
        // линии, поставка 19 §4). Начало от габарита сдвигало фазу сетки для ВСЕГО тела при правке крайнего узла:
        // щиколотку не трогали, а она стала на 2 см тоньше, потому что сдвинулась лопатка. Со смешением графа химера
        // «перещёлкивалась» бы огранкой целиком на каждом графте. Габарит теперь решает только, сколько клеток брать
        lo.y = Mathf.Floor(lo.y / cell) * cell;
        lo.z = Mathf.Floor(lo.z / cell) * cell;
