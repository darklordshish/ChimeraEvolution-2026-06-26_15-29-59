...
public abstract class AbilityData
{
    ...
    /// <summary>В ГОСТЯХ: орган надет не на своё родное шасси (`Organ.nativeChassis` задан и не совпал с шасси тела).
    /// Раскрытая копия режет себя сама — что именно, решает приём: у захвата это кап стадии (`ConstrictData`).
    /// По умолчанию ничего не режется; закрыть приём в гостях целиком — <see cref="NativeOnly"/>.</summary>
    public virtual void OnForeignChassis() { }
