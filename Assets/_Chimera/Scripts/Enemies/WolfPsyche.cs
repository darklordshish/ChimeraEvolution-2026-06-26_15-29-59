...
public class WolfPsyche : MonoBehaviour, IGrabber, IBodyStatConsumer, ICarried
{
    ...
    // драйвер. Порог срыва ударом извне — в записи Пасти (10): лёгкие удары хватку не рвут, тяжёлые рвут; нокбек, рывок
    // и прорежение стаи — отпускание по решению драйвера. Спасение по-прежнему и таран лося (нокбек)
    ...
    void UpdateGrab()
    {
        ...
        Settle(pull);
        // отпускает пинок (Knockback), рывок (BreakFree) или тяжёлый удар извне — порог в записи Пасти, срыв делает машина
    }
