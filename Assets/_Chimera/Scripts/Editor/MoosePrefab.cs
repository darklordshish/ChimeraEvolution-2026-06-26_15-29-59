...
public static class MoosePrefab
{
    ...
    public static GameObject BuildMoose()
    {
        ...
        */
        ...
        // МОЩНЫЙ таран: быстрее волчьего рывка (35 > 30) + дальний завод (18м) → догоняет убегающего волка
        // по прямой; damagePerMeter скручен (1.0), чтобы разбег не делал ваншотом.
        //     РАЗБЕГ ПОДРЕЗАН НА ТРЕТЬ (08.09, по игре): было 1.1 с ≈ 38 м при заводе с 18 м, то есть
        // ДВАДЦАТЬ метров зверь ехал уже за целью. Это читалось как «таран длинный», а не «мощный»:
        // промах наказывал не лося, а того, кто рядом стоял. 0.73 с ≈ 26 м — добегает с предельных 18 м
        // и проскакивает ещё восемь, чего хватает и на инерцию туши, и на топот-приземление в конце
        PrefabConfig.Set(charge, ("windupTime", 0.5f), ("minRange", 4f), ("maxRange", 18f),
                                          ("chargeSpeed", 35f), ("duration", 0.73f), ("damage", 22), ("damagePerMeter", 1.0f),
    ...
    /// <summary>
    /// ЛОСИНАЯ ГОЛОВА (переиспользуемая): череп + длинная ГОРБАТАЯ морда, опущенная
