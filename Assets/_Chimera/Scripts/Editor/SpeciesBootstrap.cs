...
public static class SpeciesBootstrap
{
    ...
    public static void CreateDefaults()
    {
        ...
        human.sockets = new[]
        {
            new BodySocket { name = "хребет", localPos = new Vector3(0.000f, 1.371f, 0.023f), baseEuler = new Vector3(0.000f, 0.000f, 0.000f), baseSize = new Vector3(0.470f, 0.600f, 0.240f) }, // НЕСУЩИЙ ЦЕНТР: форму даёт орган «Хребет» (chassisOnly), поэтому место больше не служебное  // ЕДИНЫЙ ПЛАН ТЕЛА (спека 2026-08-27): ось от головы назад. Числа посчитаны Anatomy/tools/reroot.py и самопроверены — тело осталось на месте
            ...
            new BodySocket { name = "нос", parent = "Пасть", attach = 0.500f, attachOffset = new Vector3(0.000f, 0.3514f, -0.3849f), sizeRel = new Vector3(0.3559f, 0.7432f, 0.6475f), formFrom = "Чутьё", formRole = PartRole.Nose }, // МОЧКА ЕДЕТ С МОРДОЙ (поставка 4 §3, 17.09): привили волчью Пасть — мочка на кончике морды оборотня, а не на лбу; доли пересчитаны от калибра Пасти так, что на родном составе мочка стоит там же до десятитысячной. АДРЕС НОСА: форму даёт орган Чутья — сменил его, сменился и нос
            new BodySocket { name = "голова", parent = "шея", attach = 0.000f, attachOffset = new Vector3(0.000f, 1.436f, -0.021f), baseEuler = new Vector3(-10.000f, 0.000f, 0.000f), sizeRel = new Vector3(1.412f, 2.016f, 1.715f), parts = new[] {
            ...
            new BodySocket { name = "шея", parent = "хребет", attach = 0.000f, attachOffset = new Vector3(0.000f, 0.839f, -0.055f), baseSize = new Vector3(0.126f, 0.134f, 0.126f), sizeRel = new Vector3(0.268f, 0.223f, 0.525f), baseEuler = new Vector3(10.000f, 0.000f, 0.000f), parts = new[] {
        ...
        // КОСТЕЙ В КОДЕ НЕТ (17.09). Форму вида даёт силуэтный граф из файла поставки модельной линии
        // `Docs/models/handoff/chelovek-graph.json`, его подхватывает `SpeciesHandoff` ниже. Нет файла —
        // вид рисуется одними местами. Анатомический скелет, лежавший здесь, признан негодным: двойная
        ...
        wolf.organs = new[]
        {
            ...
            new Organ { organName = "Волчьи ноги",   slot = "Ноги",   hotkey = "2", cost = 4, moveSpeed = 9f, dashSpeed = 30f, abilities = new AbilityData[] { new LeapData { staminaCost = 30f, damage = 12, minRange = 5f, maxRange = 6.5f, speed = 13f, up = 5f, duration = 0.5f, hitRadius = 1.3f, windupTime = 0.5f } }, visualScale = new Vector3(1f, 1f, 1.2f), visualParts = new[] {
        ...
        wolf.sockets = new[]
        {
            ...
            new BodySocket { name = "нос", parent = "Пасть", attach = 0.500f, attachOffset = new Vector3(0.000f, 0.6455f, -0.2281f), sizeRel = new Vector3(0.3623f, 0.4010f, 0.2132f), formFrom = "Чутьё", formRole = PartRole.Nose }, // МОЧКА ЕДЕТ С МОРДОЙ (поставка 4 §3, 17.09): привили волчью Пасть — мочка на кончике морды оборотня, а не на лбу; доли пересчитаны от калибра Пасти так, что на родном составе мочка стоит там же до десятитысячной. Запасные числа: при поставке их перекрывает раскладка головы. МОЧКА на конце спинки носа, выступает на 3 см и свешена вниз
        ...
        // КОСТЕЙ В КОДЕ НЕТ (17.09). Форму вида даёт силуэтный граф из файла поставки модельной линии
        // `Docs/models/handoff/volk-graph.json`, его подхватывает `SpeciesHandoff` ниже. Нет файла —
        // вид рисуется одними местами. Анатомический скелет, лежавший здесь, признан негодным: двойная
        // геометрия поверх мест, несвязный граф, десятки тысяч треугольников (кадры в `Docs/Диаграммы/Кадры/`).
        ...
        // БЮДЖЕТ ×4 (решение геймдизайнера 17.09): клетка 0.056 вместо 0.014. На силуэтном графе волка это ~2 560
        // треугольников поля при целом силуэте; ×5 рвётся у скакательного сустава, ×6 — предплечье (замер модельной
        // линии, поставка 3 §6). Прежние 0.014 держали рот и ноздри полем — теперь это ригблоки и детали Пасти.
        // КЛЕТКА 0.084 ГРАНЯМИ (выбор геймдизайнера 17.09 по тройке затенения, «третий»). Включена вместе с графом v3
        // (поставка 4): поле ноги кончается там, где нога ещё не уже 0.1 м, ниже — блоки; волк целиком ~2 100 тр.
        // На графе v2 эта клетка истончала предплечье до стебля 2.5 см — поэтому клетка и граф идут ОДНИМ заходом, а
        // раскладка головы посчитана под неё: на 0.056 глаз торчал бы из оболочки на 1.8 см.
        // С поставки 5 это зерно ОДНО НА ПРОЕКТ (`SpeciesSO.Grain`) и присваивается всем видам ниже, перед поставкой
        ...
        // ТОЛЩИНА ТЕЛА ЗМЕИ — одно число: из неё и диаметр звена, и высота цепи над землёй (радиус — брюхо на земле).
        // До 22.09 высота стояла отдельным 0.30 при толщине 0.30, и тело висело на 0.15 (поставка 13 модельной линии)
        const float snakeBody = 0.300f;
        // ЗВЕНЬЯ ЦЕПИ — числами, из которых выводится стык шеи. Шея растёт от хребта вперёд, и её `attach` — доля
        // вдоль ВСЕЙ цепи хребта: стык ровный, только если (1 − attach) × длина хребта = длина шеи. До 25.09 там стояло
        // 0.2, и при 5 звеньях хребта это совпадало с шеей (0.8 × 1.8 = 1.44 = 4 × 0.36) случайно; поставка 21 дала
        // хребту 7 звеньев, и шея уехала за голову на 0.58 м (карта тел: ЩЕЛЬ шея–хребет 0.53)
        const float snakeLink = 0.360f;
        const int snakeNeckLinks = 4, snakeSpineLinks = 7;
        const float snakeNeckAttach = 1f - (float)snakeNeckLinks / snakeSpineLinks;
        ...
        snake.organs = new[]
        {
            ...
            new Organ { organName = "Тело-хвост",           slot = "Ноги",   hotkey = "7", cost = 5, moveSpeed = 10f, dashSpeed = 20f, chassisOnly = true, digestion = true, abilities = new AbilityData[] { new LeapData { staminaCost = 20f, damage = 8, minRange = 4f, maxRange = 9f, speed = 20f, up = 2.5f, duration = 0.35f, hitRadius = 1.5f, windupTime = 0.35f } } }, // ходовая часть ШАССИ змеи: аугументом не крадётся (локомоция = свойство шасси) + ПЕРЕВАРИВАНИЕ (глотание целиком = свойство змеиного тела)
            ...
            new Organ { organName = "Хвост",                slot = "Хвост",  hotkey = "8", cost = 5, abilities = new AbilityData[] { new ConstrictData { maxStage = 3, windupTime = 0.35f, ripSelfKnock = 5f, biteInterval = 1.2f } }, nativeChassis = "Змея", visualScale = new Vector3(1f, 1f, 1f), visualSegments = 3, visualTaper = 0.82f, visualParts = new[] {
            ...
            } }, // ХВОСТ СЕГМЕНТЕН: привитый — цепочка звеньев (≈треть змеиных сегментов), масштаб под человека, а не волчий обрубок. АУГУМЕНТ игроку (обхват); запись ConstrictData: ст.3 удушения только дома, на змеином шасси (у человека кап foreignMaxStage 2). «Тело-хвост» выше — ходовая часть ШАССИ змеи, не путать
        };
        ...
        snake.sockets = new[]
        {
            ...
            new BodySocket { name = "шея", parent = "хребет", chainForward = true, attach = snakeNeckAttach, solid = true, linkDiameter = 0.215f, linkLength = snakeLink, linkTaper = 1.118f, chain = snakeNeckLinks }, // шея: от толщины головы РАСТЁТ к телу (taper > 1) — последнее звено ровно в тело
            ...
            // змеи и есть вереница звеньев; высота — радиус звена: брюхо на земле, ось та же, что у `SnakeBodyChain`
            new BodySocket { name = "хребет", localPos = new Vector3(0.000f, snakeBody * 0.5f, -1.640f), solid = true, linkDiameter = snakeBody, linkLength = snakeLink, linkTaper = 0.980f, chain = snakeSpineLinks }, // туловище: самое массивное, чуть сходит к хвосту (0.30 × 0.98⁶ = 0.266 на конце)
            // ЗВЕНЬЯ ХВОСТА МЕЛЬЧЕ ТЕЛЕСНЫХ (0.24 против 0.36) — как хвостовые позвонки у змей: при звене 0.36
            // кончик выходил тонкой прямой палочкой. ДОЛИ ЦЕПИ — С ЛИСТА (поставка 21, модельная линия, 25.09):
            // хвост был 32 % длины при 15 % на листе (у гремучников за клоакой 7–10 %), тело — 56 % при 70 %.
            // Длина переложена из хвоста в тело, общая та же 5.63 м (геймдизайнер: «змея большая, иначе как бы
            // она волков заглатывала»): хребет 5 → 7 звеньев, хвост 6 → 3. Три, а не два: двумя хвост не изогнётся
            new BodySocket { name = "Хвост", parent = "хребет", attach = 0.000f, attachOffset = new Vector3(0.000f, 0.000f, 0.000f), solid = true, linkLength = 0.240f, linkTaper = 0.670f, chain = 3 }, // хвост: подхватывает толщину тела (0.266) и уходит на конус 0.18 → 0.12
        ...
        // КОСТЕЙ В КОДЕ НЕТ (17.09). Форму вида даёт силуэтный граф из файла поставки модельной линии
        // `Docs/models/handoff/zmeya-graph.json`, его подхватывает `SpeciesHandoff` ниже. Нет файла —
        // вид рисуется одними местами. Анатомический скелет, лежавший здесь, признан негодным: двойная
        // геометрия поверх мест, несвязный граф, десятки тысяч треугольников (кадры в `Docs/Диаграммы/Кадры/`).
        ...
        moose.organs = new[]
        {
            ...
            new Organ { organName = "Копыто",         slot = "Руки",   hotkey = "1", cost = 5, abilities = new AbilityData[] { new LimbStrikeData { cooldown = 1.1f, damage = 22, knockForce = 4f, range = 1.8f, halfAngle = 60f, windupTime = 0.45f } }, visualParts = new[] {
            ...
            new Organ { organName = "Лосиные ноги",   slot = "Ноги",   hotkey = "2", cost = 5, moveSpeed = 5f, dashSpeed = 35f, dashDuration = 0.38f, abilities = new AbilityData[] { new ChargeData { staminaCost = 70f, damage = 22, damagePerMeter = 1f, knockForce = 12f, lightChargeMult = 0.25f, staggerTime = 0.5f, hitRadius = 1.8f, minRange = 4f, maxRange = 18f, chargeSpeed = 35f, duration = 0.73f, windupTime = 0.5f, stompRadius = 4f, stompStagger = 0.4f, stompForce = 13f, plowForce = 6f, plowRadius = 1.6f } }, visualParts = new[] {
            ...
            new Organ { organName = "Глотка",         slot = "Пасть",  hotkey = "5", cost = 4, abilities = new AbilityData[] { new BellowData { fearRadius = 10f, rallyRadius = 40f, fearMoraleHit = 2f, cooldown = 10f, cueTime = 1.2f } } }, // РЁВ (K2): кин-лоси в берсерк на месте, чужим страх
            ...
            new Organ { organName = "Рога",           slot = "Рога",   hotkey = "8", cost = 5, abilities = new AbilityData[] { new AntlerData { damage = 12, knockForce = 9f, bleedStacks = 2, range = 2.5f, halfAngle = 50f, windupTime = 0.35f, cooldown = 2.5f } }, visualScale = new Vector3(1f, 1f, 1f), visualParts = new[] {
        ...
        moose.sockets = new[]
        {
            new BodySocket { name = "хребет", localPos = new Vector3(0.000f, 2.360f, 0.552f), baseEuler = new Vector3(0.000f, 0.000f, 0.000f), baseSize = new Vector3(0.521f, 0.791f, 2.284f) }, // НЕСУЩИЙ ЦЕНТР: форму даёт орган «Хребет» (chassisOnly), поэтому место больше не служебное  // ЕДИНЫЙ ПЛАН ТЕЛА (спека 2026-08-27): ось от головы назад. Числа посчитаны Anatomy/tools/reroot.py и самопроверены — тело осталось на месте
            new BodySocket { name = "нос", parent = "Пасть", attach = 0.500f, attachOffset = new Vector3(0.000f, 0.2600f, -0.0633f), sizeRel = new Vector3(0.7883f, 0.7200f, 0.3376f), formFrom = "Чутьё", formRole = PartRole.Nose }, // МОЧКА ЕДЕТ С МОРДОЙ (поставка 4 §3, 17.09): привили волчью Пасть — мочка на кончике морды оборотня, а не на лбу; доли пересчитаны от калибра Пасти так, что на родном составе мочка стоит там же до десятитысячной. АДРЕС НОСА: мочку рисует орган Чутья
            new BodySocket { name = "голова", parent = "шея", attach = 0.000f, attachOffset = new Vector3(0.000f, -0.065f, 1.149f), sizeRel = new Vector3(0.677f, 0.789f, 1.018f), baseEuler = new Vector3(40.000f, 0.000f, 0.000f), parts = new[] {
            ...
            new BodySocket { name = "шея", parent = "хребет", attach = 0.000f, attachOffset = new Vector3(0.000f, -0.292f, 0.738f), baseSize = new Vector3(0.420f, 0.560f, 0.700f), sizeRel = new Vector3(0.805f, 0.708f, 0.306f), baseEuler = new Vector3(-38.000f, 0.000f, 0.000f), parts = new[] {
        ...
        // КОСТЕЙ В КОДЕ НЕТ (17.09). Форму вида даёт силуэтный граф из файла поставки модельной линии
        // `Docs/models/handoff/los-graph.json`, его подхватывает `SpeciesHandoff` ниже. Нет файла —
        // вид рисуется одними местами. Анатомический скелет, лежавший здесь, признан негодным: двойная
        // геометрия поверх мест, несвязный граф, десятки тысяч треугольников (кадры в `Docs/Диаграммы/Кадры/`).
        ...
        hog.organs = new[]
        {
            ...
            new Organ { organName = "Игломёт",           slot = "Игломёт", hotkey = "8", cost = 4, abilities = new AbilityData[] { new VolleyData { damagePerQuill = 4, bleedPerQuill = 1, slowPerQuill = 1, quills = 6, spreadAngle = 9f, speed = 22f, hitRadius = 0.35f, minRange = 6f, maxRange = 15f, windupTime = 0.5f, cooldown = 2.5f, blindAimError = 1.6f } }, visualScale = new Vector3(1f, 1f, 1f), visualParts = new[] {
            ...
            } }, // ёж НЕ догоняла, а ПИННЕР: на Э 0.5 = 3.0 — медленнее уползающей змеи (3.75), сам не догонит. Ловит КИТОМ: залп замедляет → подошёл → схватил. ПЕРЕКАТ (запись RollData): рывок «в клубке» режет иглами кого прокатил — третий профиль ног
            ...
            new Organ { organName = "Цепкая пасть",      slot = "Пасть",  hotkey = "5", cost = 4, abilities = new AbilityData[] { new BiteData { damage = 22, regenDebuff = 0.5f, regenDebuffTime = 3f, range = 1.7f, halfAngle = 60f, windupTime = 0.4f, cooldown = 0.7f }, new ConstrictData { maxStage = 1, escapeMin = 3.5f, escapeMax = 5.5f, breakRawThreshold = 7, windupTime = 0f, cooldown = 3f } }, nativeChassis = "Ёж" }, // ДОБИВАНИЕ + ПИН пастью (ст.1): та же челюсть грабит и кусает прижатую добычу. 22 (≈11 на Э 0.5) даёт ежу грабнуть-и-добить
        ...
        hog.sockets = new[]
        {
            ...
            new BodySocket { name = "хребет", localPos = new Vector3(0.000f, 0.500f, -0.050f), baseEuler = new Vector3(0.000f, 0.000f, 0.000f), baseSize = new Vector3(0.760f, 0.600f, 1.050f) }, // НЕСУЩИЙ ЦЕНТР: форму даёт орган «Хребет» (chassisOnly), поэтому место больше не служебное
            ...
            new BodySocket { name = "шея", parent = "хребет", attach = 0.000f, attachOffset = new Vector3(0.000f, -0.167f, 0.948f), baseSize = new Vector3(0.170f, 0.180f, 0.200f), sizeRel = new Vector3(0.224f, 0.300f, 0.190f), baseEuler = new Vector3(0.000f, 0.000f, 0.000f), parts = new[] { // ДОЛЯ ОТ ГОЛОВЫ (была от хребта): sizeRel считается от родителя, и при развороте её надо пересчитывать — иначе место схлопнется
            ...
            new BodySocket { name = "голова", parent = "шея", attach = 0.000f, attachOffset = new Vector3(0.000f, 0.000f, 1.001f), sizeRel = new Vector3(1.235f, 1.244f, 1.600f), baseEuler = new Vector3(0.000f, 0.000f, 0.000f), parts = new[] { // КОРЕНЬ ГРАФА: доли больше нет — у корня нет родителя, габарит читается из baseSize // ДОЛЯ ПЕРЕСЧИТАНА ПОД ШЕЮ (была 0.276/0.373/0.305 от хребта): sizeRel считается от РОДИТЕЛЯ, и при пересадке места на другого родителя старая доля схлопывает деталь — голова ужалась вчетверо вместе с пастью, носом, глазами и ушами
        ...
        // КОСТЕЙ В КОДЕ НЕТ (17.09). Форму вида даёт силуэтный граф из файла поставки модельной линии
        // `Docs/models/handoff/ezh-graph.json`, его подхватывает `SpeciesHandoff` ниже. Нет файла —
        // вид рисуется одними местами. Анатомический скелет, лежавший здесь, признан негодным: двойная
        // геометрия поверх мест, несвязный граф, десятки тысяч треугольников (кадры в `Docs/Диаграммы/Кадры/`).
        ...
        // ФОРМА ПРИХОДИТ ИЗ ПОСТАВКИ МОДЕЛЬНОЙ ЛИНИИ, А НЕ ИЗ ЭТОГО ФАЙЛА. Силуэтный граф — решение
        // формы (зона модельной линии), и хранить его копию в нашем коде значит завести второй источник:
        // приняли граф в ассет — а следующее «Создать дефолтные виды» молча вернуло старые кости.
        // Есть файл `Docs/models/handoff/<вид>-graph.json` — он и есть форма; нет файла — формы нет вовсе,
        // вид рисуется одними местами (кости и скрытия обнуляются явно: ассет помнит прежние значения)
