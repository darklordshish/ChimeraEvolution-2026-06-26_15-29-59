...
public static class BodyRules
{
    ...
    public static List<Issue> CheckData(SpeciesSO s)
    {
        ...
        foreach (var k in s.sockets)
        {
            ...
            //     УТОЧНЕНО 26.09 (спека «место — плейсхолдер», П2): место с `formFrom` открывает НЕ свой орган, а
            // орган-источник — привитое Чутьё приносит в `ямки` признак роли Pit. Такому месту графт осмыслен: «пусто,
            // пока источник не принёс форму». Без `formFrom` правило прежнее — `горб` графтом по-прежнему ошибка
            if (k.graft && BodySlots.IsPlace(k.name) && string.IsNullOrEmpty(k.formFrom))
                list.Add(new Issue
                {
                    species = s.speciesName, where = k.name, error = true,
