...
| **Орган-слот** | Анатомическое «гнездо» (глаза, пасть, руки, ноги, сердце, кожа…). Каждый слот = ветка взаимоисключающих вариантов; один вариант на слот. |
| **Шасси** | Базовый телесный план — определяет, какие слоты вообще существуют (человек, волк, …). В MVP только человек. |
| **Конструктор** | Система сборки тела из аугументов (в лаборатории). Перки поданы как **созвездия** / «небесная карта» (см. разд. 6). |
| **Родство** | Привыкание к виду, копится от убийств: **+1 за каждый УНИКАЛЬНЫЙ видо-флаг убитого тела** (шасси + каждый вид-донор с ≥1 надетым органом). Удешевляет аугументы этого вида (в MVP — только скидка). Человечность качается с убийства человеко-химер (боссы) — второй источник к «выживанию без зверя». |
...
- Выходит на арену — появляются волки
...
**6 слотов MVP (человек ↔ волк):**

| Слот (орган) | Человек | Волк |
|---|---|---|
| **Глаза/чутьё** | обычное восприятие | сумеречное зрение + нюх + слух (бандл) |
| **Пасть** | — (укуса нет) | укус: захват/добивание |
| **Руки/Когти** | оружие, инструменты | раздирающий коготь, нет оружия |
| **Ноги** | баланс, точность | скорость, рывок/прыжок |
| **Сердце** | ровная стамина | берсерк-топливо |
| **Кожа** | база | шерсть: броня |
...
- **Голова — концептуальный якорь:** в ней сходятся чувства, пасть и мозг-индикатор. В MVP сенсорика свёрнута в один слот «Глаза/чутьё» (волк отдаёт зрение+нюх+слух пакетом); post-MVP голова расщепляется — глаза/уши/нос становятся отдельными слотами под разные виды (уши летучей мыши = эхолокация, глаза змеи = термозрение). Здесь концентрируется будущая глубина перцепции (разд. 5.5).
...
- **Родство = только скидка** (в MVP). Убийства волков копят родство-волк → волчьи органы дешевеют.
...
- **Расширение пула боссом — НЕ в MVP** (Вервольф = финал зоны). Многозонное внутризабеговое расширение — v1.0+.
- **Общая модель (не только MVP):** пул = ёмкость под все аугументы (на старте занята человеком), родство = скидка по виду, человек — тоже вид. MVP — вырожденный случай: один звериный вид (волк) + человеческие тех-аугументы в Бэклоге, поэтому пул практически гейтит «сколько человека ты разменял на волка».
...
> **Не в MVP.** В MVP всего два вида (человек и волк), поэтому смена шасси нерелевантна: игрок остаётся на человеческом шасси, а волк присутствует как набор аугументов поверх человека. Совместимость не-нативных органов считается через множитель стоимости (разд. 5.2).
...
|---|---|---|
| **Лес** — старт (MVP) | волк, медведь, ёж, сова, летучая мышь, жук-олень | **Вервольф** (человек + волк) · *в MVP только волк* |
...
- **Расширение пула** + **химерный слот** — доп-слот, добавляемый к шасси. Химерные слоты = «заработанный дом» для не-нативных органов (альтернатива дорогому графту через множитель, разд. 5.2) и путь к мифическим body plans (Бэклог). **Уже релевантно со ВТОРЫМ видом** (аугументов больше, чем нативных слотов): второе сердце волк+змея, хвост-удушение, змеиная голова-яд, кожа-стелс. Стоимость постановки в химерный слот = `множитель_не-нативного_слота` (×2+, по балансу).
- **Супремум-стек:** повторяющиеся бонусы ВНУТРИ химеры (одна ось от нескольких органов) складываются НЕ аддитивно, а по **супремуму** (скаляр → `max`, булево → `OR` = поточечный join). Дубль оси не растит силу (второе сердце ≠ ×2 реген) → окупается только НОВЫМ направлением (змеиное сердце = холоднокровность) → микс вознаграждён алгебраически. Согласуется с «каждый стат = свой слот»: сегодня ось = один орган, поэтому `join == сумма`; расходятся ровно на дубле из химерного слота.
...
- **Волк:** рык, вой (вторичные способности пасти/голоса).
- Полный органный список слотов (сердце/лёгкие/печень/желудок/… + шасси) разворачивается с новыми видами.
- **Химеризация шасси (далёкое будущее):** свободная перестройка телесного плана — змеиная голова вместо хвоста, несколько голов, кентавр-конфигурации, мифические существа. За пределами «смены шасси» — переизобретение body plan. Встаёт в ту же формулу стоимости как «очень дорого», а не как особый случай.
