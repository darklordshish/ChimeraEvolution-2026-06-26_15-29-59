...
| **Экспрессия** | Насколько раскрыты гены зверя в теле. Игрок — авто от родства (ЛИНЕЙНО 0→100 → мощь ×1…×2); NPC — фикс (босс 2 = потолок игрока, природный волк 0.45, змея 0.5). Универсальное тело: NPC собраны из тех же органов × экспрессия. |
...
- Финал зоны — босс-суперхимера (собирается конструктором: оборотень или случайная химера; вервольф как отдельный вид снят 11.09)
...
- **Пасть = развилка билда:** волчья (кровотечение + вой + захват) ↔ змеиная (урон + яд). Причём вой-стан — козырь против змеиного обхвата (рвёт его на ст.1–2): взял яд — потерял страховку. *(Вампиризм волчьей Пасти снят 12.09: он был хардкодом грани игрока и доставался ЛЮБОЙ Пасти, из-за чего развилки не было вовсе. Лечение укусом вернётся полем органа вместе с видом-носителем; низкий урон волчьих клыков компенсирует кровотечение.)*
...
### 5.10 Идентичность по составу, психика-диспатч и эволюция NPC (РЕАЛИЗОВАНО, 2026-07-31)

Тело собирает из состава не только функции, но и **кто ты по поведению** — и это замыкает химеризацию в живую петлю.

- **Идентичность = ДОЛЯ вида в массе тела.** `MostKin` = доминантный вид (null = истинная химера, кин ни к кому). `Regard(наблюдатель, цель)` признаёт цель по её ДОМИНАНТЕ, не по шасси: «химеры-змеи» нет — есть **змея-по-идентичности** (собранная как угодно) и **истинная химера**. Три градации признания (слабое/среднее/сильное) — люфт под социальное встраивание.
- **Диспатч психики по составу.** Существо получает психику своей доминанты (волк→волчья, змея→змеиная…); истинная химера и доминанта-Человек → **химера-альфа** (агрессивный апекс, враг всем). Психика-компонент = маркер идентичности: виды узнают друг друга по нему. Психику больше не вешают руками — её ставит состав.
- **Эволюция NPC.** Звери убивают друг друга → копят родство к виду жертвы → **стохастически надевают её органы** (шанс = родство) → доминанта переваливает → **метаморфоза психики** (волк, обожравшийся ежей, ведёт себя как ёж). Шасси не меняется — физический якорь: «лось-ёж» = высокий ёж-танк с лосиным ростом, но без игломёта (уникальный слот). Родство — единая ось (мягкое открытие веток убийствами: «изучил вид, убив его»). Человек вне эволюции = эксклюзив босса-суперхимеры. Лес эволюционирует сам; интенсивность тюнится `EvolutionConfig`.
...
> **Фундамент супер-химеры-босса — ГОТОВ (2026-07-31).** Босс-суперхимера строится «на готовом»: **морфология** (модель из состава), **эволюция** (химера, собранная убийствами), **диспатч психики** по составу (разд. 5.10). Останется прикрутить модуль боссовости (фазы/вой, часть — с вервольфа) — босс не с нуля. *(Модуль `Bossness` прикручен 11.09; фазы и вой — дальше.)* Это же и путь к Химере 0.

### Вервольф (MVP)

> ⛔ **СНЯТ 11.09.2026** решением геймдизайнера: *«у нас готовая химеризация, мы просто не дошли до
> босса-химеры… вервольфа просто выкинем, он не нужен вообще как выделенная единица»*. Босс-суперхимера
> будет собираться из конструктора, а не жить отдельным видом со своими психикой, префабом и флагом
> «всё надето с рождения». Раздел ниже оставлен как история замысла: нарратив «первый заражённый
> коллега» свободен для будущей суперхимеры.
>
> **Замена — тем же днём.** Босс собирается рецептами `BossChimeraSpawner` поверх модуля `Bossness`:
> **оборотень** — чистый зверолюд, человек + весь набор одного случайного вида + два химерных слота «с приколами»
> (вервольф — его исход, когда выпал волк), или **случайная химера**. Два слота — решение геймдизайнера.
...
**Реализовано в прототипе.** Босса собирает тот же конструктор, что и игрока (`ChimeraFactory`), а боссовость — отдельный модуль (`Bossness`): человеческое шасси + полный сет одного вида × экспрессия 2 (= потолок игрока на 100 родства) + два химерных слота + вечная ярость (+урон/+скорость/−защита) — «быстрый убийца, не танк». Отдельного вида-вервольфа больше НЕТ: «оборотень» — это рецепт спавнера, и вервольф выходит из него, когда донором выпал волк. Приёмы — от состава. Массивная туша (`Massive`): обхват игрока держит его на стадию слабее. Призыв по родству ≥75 (автоспавн пока выключен); **первое убийство типа → +4 пула + химерный слот** (разд. 10). Укус-вампиризм и вой-призыв стаи остались в модуле боссовости атавизмами — выключены до решения по механикам боссов.
...
|---|---|---|
| **Лес** — старт | **волк · змея · лось · ёж (РЕАЛИЗОВАНЫ)** · сова (в плане — эхолокация) | **Оборотень** (человек + весь набор лесного вида) или случайная химера — суперхимера-коллега |
...
**Биом = павильон лаборатории (решение геймдизайнера 23.09).** Каждый сектор-биом — отдельный павильон-купол
«ковчега», и Лес — первый из них; остальные биомы строятся так же (тот же генератор, другой набор видов, рельефа и
погоды):
- **купол со своим небом:** светильники солнца и луны ходят по своду, ночью светится мох-«звёзды»; сутки и погода
  у павильона свои (у Леса — тёплый биом без снега: ясно, дождь, туман на рассвете и закате);
- **по периметру — скалы, ангарные двери** ведут в соседние секции комплекса;
- **в центре — станция лаборатории:** конструктор, арена, клетки, наблюдательный пункт;
- **масштаб — около 2 км в поперечнике** (настраивается). Экосистема должна казаться правдоподобно большой: у зверей
  свои территории и ареалы обитания, значит, места должно быть много. Встречи держатся не теснотой, а устройством
  ареалов — логова, водопои, кормёжка, тропы.

Сектор из разд. 3 и 4 = павильон: отключение сигнализации идёт павильон за павильоном.
...
**Морфология из состава (кубический задел, РЕАЛИЗОВАНО 2026-07-31):** химеры собираются из данных — модель = скелет-якоря шасси + куб-части органов (`MorphBuilder`), пересборка при химеризации на лету (что надел — то и видно). Пока кубы (грубо, единый стиль); детализация и красивое смешение морфологий — за модельным проходом (Blender/скин-меши поверх работающей структуры). Тело-как-данные распространилось и на визуал — третья ось после функций и психики.
...
Убийство боссов-суперхимер → **разовая награда за ТИП босса** (первое убийство; повторные того же типа дают только обычное родство). Постоянна в сейве (перманентность — с мета-сейвом; до него прототипируется В СЕССИИ, как родство/`AffinityTracker`). **Первая суперхимера — босс леса** (коллега: человек-шасси + сыворотка + звериные ауги; оборотень или случайная химера) — на нём и обкатывается луп; реализация — компонент `SuperBossReward`, его вешает модуль боссовости (тип + пул + выдача химерного слота), отдельно от родства (родство даёт КАЖДОЕ тело).

- **Расширение пула** + **химерный слот** — доп-слот, добавляемый к шасси. Химерные слоты = «заработанный дом» для не-нативных органов (альтернатива дорогому графту через множитель, разд. 5.2) и путь к мифическим body plans (Бэклог). **РЕАЛИЗОВАНО в прототипе** (+4 пула + 1 слот за первое убийство босса-суперхимеры, цена органа в слоте ×2, лимит 1): второе сердце волк+змея работает, «Змеиный хвост» с обхватом живёт именно здесь.
...
|---|---|---|
| MVP | Одна арена, волки, босс-суперхимера из конструктора | Базовый цикл, конструктор, родство, шкала мозга |
...
- Босс-суперхимера — собирается конструктором (вервольф снят 11.09)
...
**Сверх того (август 2026) — ФУНДАМЕНТ ПОД ХИМЕРНУЮ ФОРМУ.** Месяц ушёл не в геймплей, а под него: чтобы химера могла быть СМЕСЬЮ, все виды должны быть построены по одному закону. Сделано: **сокет-план** (имя места = `Organ.slot`, одно имя держит и механику, и визуал); **скелет стал органом**, а не служебной пустотой — прежде несущую анатомию рисовал покров, и шесть мест из десяти висели на пустоте; **единый план тела** — ось от головы у всех пяти видов (прежде каждый вид имел свою топологию, и смешивать их было нечего); **карта тел как детектор** — строит существо настоящим билдером и меряет результат, поэтому разойтись с игрой не может. Решён источник формы: **клетка** — слот описывается таблицей сечений одинаковой размерности у всех видов, и меш химеры получается покомпонентным средним таблиц, а не сшиванием геометрии. Волк снят в клетках целиком. **Смешение намеренно не включено**: сперва чистые виды, иначе усреднение законсервирует кривизну исходников и виноватой покажется смесь.

Где проект стоит сейчас, что в работе и что дальше — **`Docs/СТАТУС.md`** (точка входа, обновляется при смене этапа).
...
- **Вервольф → суперхимера — РЕАЛИЗОВАНО (2026-09-11):** босс-коллега как человек-шасси + звериные ауги — рецепты оборотень / случайная химера (`BossChimeraSpawner` + модуль `Bossness`), награда за тип (разд. 10).
