...
## Уточнение к спеке (осознанное, поднято при разборе API)

Спека §2.2 предлагала таргет «не-кин (`Regard(other,me) < Weak`)». **В плане альфа таргетит ВСЕХ живых без кин-фильтра.** Причина: `Regard(other, me)` смотрит `me.Chassis`, а у истинной химеры шасси всё ещё какой-то вид (напр. Человек) — тогда чистый человек-NPC читается как `Strong` и ошибочно попал бы в «кин». Для фолбэка «монстр для всех» (замысел пользователя) кин-фильтр вырождается и не нужен. **Кин-фильтр понадобится ВИДОВЫМ модулям (#4b-3)** — там он применяется правильно (по своему виду, а не по шасси химеры). MVP-альфа проще и семантически вернее: ближайший живой = цель.
