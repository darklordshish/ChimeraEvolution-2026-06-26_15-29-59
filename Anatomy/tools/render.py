...
# ЭТАЛОН ГРУЗИТСЯ ИЗ ФАЙЛА ВИДА, снятого с фотографии (см. etalon/body.py). Прежде он был вписан
# сюда же руками, из краниометрических таблиц и правил для художников, — и оказался перевёрнут в
# главном: у волка голова ВЫШЕ линии спины, а выдуманный контур вёл линию от холки вниз. Модель
# сошлась с ним на 2.1 см и всё равно читалась неправильно. Эталон, который можно сочинить, сочинят
_et_path = os.path.join(SPECIES_DIR, 'etalon', 'body.py')
if os.path.exists(_et_path):
    _et = {}
    exec(open(_et_path, encoding='utf-8').read().split("if __name__")[0], _et)
    ETALON_TOP, ETALON_BOT = _et['TOP'], _et['BOT']
else:
    ETALON_TOP = ETALON_BOT = []

def compare(F, lo, cell, path):
    if not ETALON_TOP:
        print('эталон тела не найден — сравнивать не с чем'); return
    ...
    zones = [('круп', -0.60, -0.30), ('поясница', -0.30, 0.05), ('грудь/холка', 0.05, 0.40),
             ('плечо/загривок', 0.40, 0.60), ('шея', 0.60, 0.80), ('голова', 0.80, 1.20)]
