...
def audit(path, slots, places):
    ...
    # 5. ОРГАН БЕЗ МЕСТА — механика работает, видно ничего не будет
    for o in organs:
        if o['_slot'] and o['_slot'] not in by:
            bad(o['_name'], 'нет места под слот «%s» — орган невидим' % o['_slot'])

    # 6. МЕТРЫ У ДОНОРА — правило спеки морфологии по идентичности
    for o in organs:
        off = vec(o.get('visualOffset'))
        if off != (0.0, 0.0, 0.0):
            bad(o['_name'], 'visualOffset в МЕТРАХ %s — донорские данные обязаны быть в долях' % (off,))

    # 7. МЕСТО БЕЗ ФОРМЫ — фикция. Пока хребет был служебным, несущую анатомию рисовал ПОКРОВ,
