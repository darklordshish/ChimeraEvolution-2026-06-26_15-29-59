...
# Отчёт на русском, обойтись ASCII нельзя, поэтому вывод переопределяется здесь
...
def audit(path, slots, places):
    ...
    # ── ЕДИНЫЙ НАБОР МЕСТ: не замечания, а СТАТУС. Печатать «не так» ошибкой значит держать отчёт
    # вечно красным — а красное, которое горит всегда, перестают читать
    #     ПРОВЕРЯЕТСЯ ГРАФ МЕСТ, А НЕ КОСТЕЙ. Прежняя редакция спрашивала корень со СКЕЛЕТА («у вида
    # со скелетом ось живёт в костях»), и проверка проходила у всех пятерых, ничего не проверяя: корень
    # мест и корень костей — разные графы, у волка они и сегодня расходятся (места на хребте, кости от
    # черепа), и это не мешает. Спека 11.09 требует единого корня МЕСТ — его и спрашиваем
    spec = []
    root = roots[0] if len(roots) == 1 else None
    spec.append(('И1 корень = хребет', root == 'хребет', 'корень «%s»' % (root or '—')))

    # И2: ОСЬ ЛИНЕЙНА ПО ВЕТВЯМ, А У КОРНЯ ИХ ДВЕ. При корне-хребте позвоночник обязан ветвиться ровно
    # один раз — вперёд к голове и назад к хвосту; требовать «не больше одного ребёнка» от корня значит
    # объявлять верную структуру ошибкой (так и было 11.09: четыре вида из пяти горели «хребет → шея,
    # Хвост»). Ниже по ветке ветвлений быть по-прежнему не должно
    axis = [x for x in sockets if x['_name'] in AXIS]
    branchy = []
    for a in axis:
        kids = [k for k in axis if unesc(k.get('parent')) == a['_name']]
        limit = 2 if a['_name'] == root else 1
        if len(kids) > limit:
            branchy.append('%s → %s' % (a['_name'], ', '.join(k['_name'] for k in kids)))
    spec.append(('И2 ось линейна по ветвям', not branchy,
                 '; '.join(branchy) if branchy else 'ветвлений сверх корневого нет'))
...
def main():
    ...
    # ПРОГРЕСС отдельной строкой: «сделано» должно быть числом, а не ощущением. С 11.09 — 5 из 5
    print('ЕДИНЫЙ НАБОР МЕСТ (спека 2026-09-11): %d из %d видов' % (migrated, seen))
    return 0
