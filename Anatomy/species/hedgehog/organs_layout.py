...
def paw(node_name, heel, toe_z, width, height, claws):
    ...
    (лист: крупные когти на всех четырёх лапах). heel — (x, z) начала лапы, toe_z — кончик пальцев. r6: пясть 14 см, а не
    пластина, пятка на 9 см утоплена в поле лапы — кисть вырастает из лапы, а не стоит на ней, торец пятки не виден."""
    ...
    for i in range(claws):
        k = (i - (claws - 1) / 2)
        dx = k * width * 0.30
        parts.append(_WL.part(n, 'клин', (x + dx, 0.028, toe_z + 0.020 - 0.012 * abs(k)), (0.024, 0.026, 0.08),
                              (0.22 * k, -0.50, 1.0)))   # r6: когти к земле, видны в профиль; ниже — уходят под землю (матрица И3)
    return parts
...
def fore_paws():
    ...
    return dict(slot='Руки', organ='Ежиные лапы', parts=paw('предплечье', (n['b'][0], n['b'][2] - 0.09), n['b'][2] + 0.12,
                                                           0.17, 0.14, 4))
...
def hind_feet():
    ...
    return dict(slot='Ноги', organ='Ежиные ноги', parts=paw('голень', (n['b'][0], n['b'][2] - 0.09), n['b'][2] + 0.16,
                                                           0.14, 0.14, 4))
