...
КИСТЬ — точка свапа и ближе всего к камере. Свой блок `кисть` (рукавица: запястье, широкая ладонь, пальцы подогнуты к
ладони); `лопата` на этом месте анфас читалась шипом. Ладонь смотрит к бедру (−X у правой руки) — туда и +Y блока. Длина кисти 0.19 (канон: 0.8 лица), ширина по пястью 0.10, толщина 0.048.
ПРЕДПЛЕЧЬЕ атлета 0.10 × 0.08 у локтя (к запястью брусок сходит до 0.072 × 0.066), ГОЛЕНЬ у лодыжки 0.085 × 0.095.
...
def arm():
    ...
    forearm = bar(n, G.v_add(G.ELBOW, G.v_mul(G.v_unit(G.v_sub(G.WRIST, G.ELBOW)), 0.03)), G.WRIST, 0.100, 0.082, 0.0)
    ...
    # блок `кисть`: +Y — ладонь (пальцы подогнуты к ней), ладонь смотрит к бедру (−X у правой руки); ширина по пястью — вперёд-назад
    hand = piece(n, 'кисть', centre, (0.100, 0.048, hand_len), d, (-1.0, 0.0, 0.0))
    return dict(slot='Руки', organ='Кисть', parts=[forearm, hand])


def leg():
    n = node_by_name('голень')
    shin = bar(n, G.SHIN_END, G.v_add(G.ANKLE, (0, -0.01, 0)), 0.085, 0.095, 0.06)
