...
# губы садятся НА МОРДУ (узел графа `морда`, 23.09): центр валика — на поверхности морды по оси, наружу выходит ~6 мм.
# ...при утопленном центре валик серединой тонул в морде, а концы торчали по её скруглению — «клыки» (третья рецензия 23.09);
# валик уже и тоньше, чтобы концы не выходили
LIPS = {0.084: 0.050, 0.042: 0.032, 0.021: 0.026}
SURFACE = {
    0.084: dict(eye_front=0.050, face_front=0.048, ear_side=0.056),
    # ЗАМЕР — НА ОБОЛОЧКЕ ИЗ `main` (узлы сетки на ±полклетки, по оси лица — ребро; письмо механик 23.09e §2). Мерить на своей
    # ветке без их сетки нельзя: середина лица у них выдвинута сильнее, и губа поставки 17 целиком ушла в лицо, глаз вышел на
    # 3 мм. Лучом по граням (`Bands.RayZ`), 23.09, отдельным редактором на чистом worktree от `origin/main`:
    # 0.042 — лоб над глазом 0.094, у глаза 0.088, скула 0.085; лицо у носа 0.089–0.093; у рта по оси 0.103, сбоку 0.100.
    # Глаз передом вровень с надбровьем (центр 0.083), губа центром на морде (0.101)
    0.042: dict(eye_front=0.087, face_front=0.093, ear_side=0.071),
    # 0.021 (прототип «мельче клетка + прореживание»): лоб 0.094, у глаза 0.091, лицо у носа 0.096–0.100, у рта сбоку 0.100
    0.021: dict(eye_front=0.089, face_front=0.098, ear_side=0.074),
}
