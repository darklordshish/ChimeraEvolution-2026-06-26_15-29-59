...
SURFACE = {
    ...
    0.042: dict(eye_front=0.077, face_front=0.082, ear_side=0.077),   # перемерено 22.09 под череп атлета (сечение 0.93)
}
...
def layout(cell):
    ...
    # впереди лица). Блок `клин` широким торцом вниз (ноздри 0.034), узким вверх (переносица), наклон 25° от отвеса
    ...
    nose_world = (0.0, 1.684, s['face_front'] + 0.002)
    nl, nu, nf = local(nose_world)
    nose_size = (0.030, 0.020, 0.050)
...
def senses():
    return [
        ...
        # Поворот (−115, 0, 180): +Z блока — вверх и назад на 25° от отвеса, +Y — вперёд из лица
        dict(role='Nose', block='клин', offset=[0, 0, 0], scale=[1, 1, 1], euler=[-115, 0, 180]),
    ]
