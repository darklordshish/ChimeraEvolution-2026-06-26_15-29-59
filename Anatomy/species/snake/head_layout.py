...
MUZZLE_BLOCK = 'булава'  # было `клин` (до 25.09)
MUZZLE_H = 0.75         # высота рыла к высоте головы (было 0.55)
...
def layout():
    ...
    # затылка по `rattlesnake_head_front.jpg`), торец тупой и закруглённый — блок `булава` (держит ширину, скругляет торец)
    m_a0, m_a1 = 0.30, L
    m_len = m_a1 - m_a0
    # ...и итерация 4: рыло торчало из черепа узкой «трубкой» — череп закруглялся и сужался ДО начала рыла, а рыло было
    # ниже темени (ступенька в профиль). Рыло шириной как у глаз и высотой почти во всю голову, череп заходит в него
    m_w, m_h = round(W * G.FRONT['у_глаз'], 3), round(H * MUZZLE_H, 3)
    ...
    places = [
        place('Пасть', 1.0, m_a, m_b, 0.0, (m_w, m_h, round(m_len, 3)), note='рыло: блок `булава`, тупое и закруглённое'),
