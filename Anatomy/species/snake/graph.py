...
Z −0.19…0.39, ось на высоте цепи, ширина 0.24): голова не должна отрываться от шеи, которую ставит код.
...
AXIS_Y = 0.15                 # высота оси цепи (`SnakeBodyChain.height`) = радиус звена: брюхо ложится на землю
# ПОСТАВКА 13 (22.09): было 0.30, и змея висела над землёй на 0.15 (замер карты тел 17.09, поставка 7 §5). Механики
# ставят высоту цепи из радиуса звена тем же заходом (их письмо 22.09 §6) — голова и тело опускаются вместе.
# Голова по-прежнему ПО ЦЕНТРУ оси: её высота 0.19 против 0.30 у звена, низ головы в 0.05 над землёй — так между
# головой и шеей нет ступеньки, а рыло чуть приподнято, как у ползущей змеи
...
HEAD_W, HEAD_H = 0.34, 0.19        # ширина по щекам 0.34 (25.09; было 0.30 — щёки всего в 1.4 раза шире шеи 0.215, у гремучника 1.5–1.8)
FRONT = {'сзади': 1.00, 'у_глаз': 0.59, 'у_рыла': 0.55}
SKULL_END_Z = 0.17            # конец черепа: внутри рыла (было 0.10 — у его начала)
...
def build_nodes():
    ...
    # ЧЕРЕП ЗАХОДИТ В РЫЛО (25.09): конец на 0.10 закруглялся до начала рыла, и сверху между ними был перехват
    b = (0.0, AXIS_Y - 0.005, SKULL_END_Z)
