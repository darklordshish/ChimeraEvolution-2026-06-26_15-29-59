...
def layout():
    ...
    SURFACE_GROWTH_AT_0084 = 0.057 - 0.039        # рост оболочки против расчёта узла (замер волка, поставка 4)
    # ЗАМЕР, А НЕ РАСЧЁТ: поверхность оболочки узла `голова` вбок в точке глаза на клетке 0.084 — 0.152 м (`ProbeM.SurfaceX`
    # по вершинам; расчёт с поправкой волка давал 0.163, анфас — 0.164). Глаз шириной 3.5 см садится центром на 4 мм
    # наружу: треть в черепе, выступ ~2 см. По анфасу (0.164) он висел бы в 5 мм от кожи
    SURFACE_AT_EYE_0084 = 0.152
    eye_lat = round(SURFACE_AT_EYE_0084 + 0.004, 3)
    ...
    top = hn['r0'] * hn['depth'] + SURFACE_GROWTH_AT_0084
