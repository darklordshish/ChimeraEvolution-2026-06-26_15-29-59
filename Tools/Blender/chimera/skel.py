...
def _order(defs):
    """Расставить кости так, чтобы родитель шёл раньше ребёнка. Топологическая сортировка.

    ЗАЧЕМ. Порядок СТРОК в файле данных не должен кодировать граф. Пока он его кодировал, смена
    корня требовала физически двигать блоки текста — а корень у нас поменялся ровно один раз и
    сразу больно: спека 27.08 велит корень `голова`, а оболочка черепа импортируется в конце файла,
    после позвоночника. Сортировка снимает это ограничение у всех видов навсегда.

    Цикл в графе ловится здесь же и с именами: без проверки он повесил бы сборку молча."""
    def deps(d):
        # ЗАВИСИМОСТЕЙ ДВЕ, НЕ ОДНА. Кость ждёт родителя, а МЫШЦА ждёт ещё и кость второго
        # крепления: её конец считается долей вдоль той кости, и раньше неё мышцу не поставить
        r = [d['parent']] if d.get('parent') else []
        if d.get('end'):
            r.append(d['end'][0])
        return r

    left = {d['name']: d for d in defs}
    done, out = set(), []
    while left:
        ready = [n for n, d in left.items() if all(x in done for x in deps(d))]
        if not ready:
            raise ValueError('ЦИКЛ В ГРАФЕ КОСТЕЙ либо родитель вне списка: %s'
                             % ', '.join('%s←%s' % (n, left[n].get('parent')) for n in sorted(left)))
        for n in sorted(ready, key=lambda n: [d['name'] for d in defs].index(n)):
            out.append(left.pop(n))
            done.add(n)
    return out
