...
С 24.09 ЭТО ПРОВЕРКА, А НЕ СПОСОБ МЕРИТЬ. Механики поставили постоянную решётку (`54679f7`, сторож `LatticePhaseTests`):
узлы по Y и Z на кратных клетки в системе тела, габарит решает только число клеток. Прогон по 4 фазам на пяти видах
(24.09): топология одна, вершины расходятся не больше чем на 0.04 мм (шум float). Один кадр теперь честен, средние по
фазам не нужны. Скрипт оставлен сторожем: разброс снова стал заметным — фазу опять двигает габарит или что-то ещё.
Ниже — как было до 24.09 и зачем скрипт писался.
